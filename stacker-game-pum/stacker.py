...
stage1 = {
    "MAX_WIDTH":(1,)*2 + (2,)*3 + (3,)*5,
    "SPEED_RATIO":10,
    "CHANNEL": 1
}
STAGES = [
    {"MAX_WIDTH":(1,)*1 + (2,)*2 + (3,)*7,
     "SPEED_RATIO":10,
     "BLOCK_SHOW": 1},
    {"MAX_WIDTH":(1,)*1 + (2,)*2 + (3,)*7,
     "SPEED_RATIO":9,
     "BLOCK_SHOW": 0},
    {"MAX_WIDTH":(1,)*1 + (2,)*3 + (3,)*6,
     "SPEED_RATIO":8,
     "BLOCK_SHOW": 1},
    {"MAX_WIDTH":(1,)*2 + (2,)*3 + (3,)*5,
     "SPEED_RATIO":7,
    ...
    {"MAX_WIDTH":(1,)*2 + (2,)*3 + (3,)*5,
     "SPEED_RATIO":6,
     "BLOCK_SHOW": 1},
    {"MAX_WIDTH":(1,)*2 + (2,)*4 + (3,)*4,
     "SPEED_RATIO":5,
     "BLOCK_SHOW": 0},
    {"MAX_WIDTH":(1,)*3 + (2,)*3 + (3,)*4,
     "SPEED_RATIO":4,
     "BLOCK_SHOW": 1},
    {"MAX_WIDTH":(1,)*5 + (2,)*2 + (3,)*3,
     "SPEED_RATIO":3,
     "BLOCK_SHOW": 0},
    {"MAX_WIDTH":(1,)*6 + (2,)*2 + (3,)*2,
     "SPEED_RATIO":2,
     "BLOCK_SHOW": 1},
    {"MAX_WIDTH":(1,)*7 + (2,)*2 + (3,)*1,
     "SPEED_RATIO": 1,
     "BLOCK_SHOW": 0},
]
...
class SlackerTile:
    ...
    def draw(self, max_x_offset=2):
        ...
        rect = pygame.Rect((self.x+self.get_xoffset(max_x_offset)) * self.SIZE, (self.y+self.get_yoffset()) * self.SIZE, self.SIZE, self.SIZE)
        pygame.draw.rect(self.screen, color, rect)
        pygame.draw.rect(self.screen, self.BG, rect, self.SIZE / 11)

    def draw_grid(self, max_x_offset=2):
        """Draw the tile."""
        color = self.BG
        rect = pygame.Rect((self.x+self.get_xoffset(max_x_offset)) * self.SIZE,
                           ...
                           self.SIZE, self.SIZE)
        pygame.draw.rect(self.screen, color, rect)
        pygame.draw.rect(self.screen, (239, 41, 41), rect, self.SIZE / 11)
...
class Slacker:
    ...
    """
    BOARD_SIZE = BOARD_WIDTH, BOARD_HEIGHT = 10, 10
    SCREEN_SIZE = SCREEN_WIDTH, SCREEN_HEIGHT = 800, 800
    TILE_SIZE = 80
    CURRENT_PATH = os.path.dirname(os.path.abspath(__file__))

    COLOR_MAJOR = TANGO['Sky Blue']
    COLOR_MINOR = TANGO['Sky Blue']
    BG_COLOR = TANGO['Aluminium'][5]
    #ICON = pygame.image.load(data('icon.png'))
    STAGE_INDEX = 0
    #MAX_WIDTH = (1,)*2 + (2,)*3 + (3,)*5
    MAX_WIDTH = STAGES[STAGE_INDEX]["MAX_WIDTH"]
    STAGES = []
    COLOR_CHANGE_Y = 5  # blocks below which are displayed in the alternate color
    WIN_LEVEL = 10
    WIN_SPEED = 100
    INTRO, PLAYING, LOSE, WIN, SCORE_W, SCORE_L = range(6)
    stime = None
    TRIGGER = False
    STAGE_SCORE = 0
    ATTEMP_TIME = 0
    ...
    def __init__(self, restart=False):
        ...
        pygame.mixer.music.load(self.CURRENT_PATH +'/bensound-buddy.mp3')
        ...
        self.speed = 200 + randrange(5)
        self.speed_ratio = 10
        self.width = self.MAX_WIDTH[-1]
        self.y = self.BOARD_HEIGHT - 1
        self.block_display = 1


    def draw_grid(self):
        for y, row in enumerate(self.board):
            for x, block in enumerate(row):
                SlackerTile(self.screen, x, y, state=self.game_state).draw_grid()

    def draw_text(self, string, height):
        """Width-fit the string in the screen on the given height."""
        font = pygame.font.Font(self.CURRENT_PATH +'/VT323-Regular.ttf',
    ...
    def score(self):
        sscore = "SCORE: " + str(self.STAGE_INDEX*10 + self.STAGE_SCORE)
        if pygame.time.get_ticks() // 820 % 2:
            self.draw_text(sscore, 0.5)
    ...
    def draw_board(self):
        ...
        for y, row in enumerate(self.board):
            for x, block in enumerate(row):
                if block:
                    if self.game_state == self.WIN or self.game_state == self.LOSE:
                        SlackerTile(self.screen, x, y, state=self.game_state).draw()
                    if self.y == y:
                        SlackerTile(self.screen, x, y, state=self.game_state).draw()
                    if self.block_display == 1 and self.y < y:
                        SlackerTile(self.screen, x, y, state=self.game_state).draw()
    ...
    def update_screen(self):
        ...
        if self.game_state == self.INTRO:
            ...
            self.draw_board()
        if self.game_state in [self.SCORE_W, self.SCORE_L]:
            self.score()
        pygame.display.flip()
    ...
    def setStateWIN(self):
        ...
        self.STAGE_INDEX += 1
        self.speed_ratio = STAGES[self.STAGE_INDEX]["SPEED_RATIO"]
        self.block_display = STAGES[self.STAGE_INDEX]["BLOCK_SHOW"]
        self.MAX_WIDTH = STAGES[self.STAGE_INDEX]["MAX_WIDTH"]

    def setStateLOSS(self):
        self.__init__(restart=True)
        self.speed_ratio = STAGES[self.STAGE_INDEX]["SPEED_RATIO"]
        self.block_display = STAGES[self.STAGE_INDEX]["BLOCK_SHOW"]
        self.MAX_WIDTH = STAGES[self.STAGE_INDEX]["MAX_WIDTH"]
            ...
            if self.game_state == self.INTRO:
                ...
                for event in pygame.event.get():
                    if event.type == pygame.QUIT:
                        ...
                        if self.game_state == self.LOSE:
                            self.setStateLOSS()
                if (time.time()-self.stime) > 5:
                    if self.game_state == self.WIN:
                        self.game_state = self.SCORE_W
                        self.RETRY_FLAG = False
                        self.stime = time.time()
                    if self.game_state == self.LOSE:
                        self.game_state = self.SCORE_L
                        self.ATTEMP_TIME += 1
                        self.RETRY_FLAG = True
                        self.stime = time.time()

            elif self.game_state == self.SCORE_W:
                self.STAGE_SCORE = 10
                self.score()
                self.RETRY_FLAG = False
                self.ATTEMP_TIME = 0
                for event in pygame.event.get():
                    if event.type == pygame.QUIT:
                        ...
                        self.setStateWIN()
            elif self.game_state == self.SCORE_L:

                self.STAGE_SCORE = (9-self.y)
                self.score()
                ...
                for event in pygame.event.get():
                    if event.type == pygame.QUIT:
                        ...
                        if self.RETRY_FLAG and self.ATTEMP_TIME == 3:
                            self.STAGE_SCORE = 0
                            self.ATTEMP_TIME = 0
                            self.STAGE_INDEX = 0
                        self.setStateLOSS()
